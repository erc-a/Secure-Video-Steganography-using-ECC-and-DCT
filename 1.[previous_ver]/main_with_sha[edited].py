...
if __name__ == "__main__":
    ...
    # Get path of the current script (ensures local behavior)
    script_dir = os.path.dirname(os.path.abspath(__file__))
    key_dir = os.path.join(script_dir, 'keys')
    os.makedirs(key_dir, exist_ok=True)

    priv_key_path = os.path.join(key_dir, 'bob_private_key.pem')
    pub_key_path = os.path.join(key_dir, 'bob_public_key.pem')
    ...
    if not os.path.exists(priv_key_path) or not os.path.exists(pub_key_path):
        ...
        try:
            ...
            print("  Kunci ECC Bob berhasil dibuat.")
            print(f"    - Private: {priv_key_path}")
            print(f"    - Public : {pub_key_path}")
        except Exception as e_key_save:
            print(f"  ERROR: Gagal menyimpan kunci ECC Bob: {e_key_save}")
            exit()
    else:
        print("  Menggunakan kunci ECC Bob dari folder lokal.")
    ...
    try:
        ...
        print(f"  ERROR: Gagal memuat kunci ECC Bob: {e_key_load}")
        exit()
